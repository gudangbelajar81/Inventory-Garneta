  ...
  let navigate = () => {};
  let window = { innerWidth: 1024, toggleAppDrawer: () => {} };
  let document = { getElementById: (id) => ({ innerHTML: '', querySelectorAll: () => [], textContent: '' }) };
        ...
        grid.innerHTML = currentMenus.map(([key, label]) => {
          ...
          return `
            <div onclick="document.getElementById('app-drawer-modal').classList.add('hidden'); state.route='${key}'; renderShell(); render();" 
                 style="cursor: pointer; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; gap: 6px; transition: transform 0.2s ease;"
  ...
  if (formPayroll) {
    formPayroll.onsubmit = async (e) => {
      ...
      const routeIcons = {
        ...
        penjualan: "💰", laporan: "📊", statistik: "📈", audit: "🔍",
        users: "👥", gaji: "💸", settings: "⚙️", "neural-hub": "🤖"
      };
      ...
      const views = { dashboard, "neural-hub": neuralHub, barang, supplier, pembelian, ngitung, riwayat, kalkulator, penjualan, laporan, statistik, audit, users, gaji, settings };
          ...
          ">
            ...
            <div style="display: flex; gap: 8px; margin-bottom: 8px; align-items: stretch;">
              <div style="flex: 1; display: flex; flex-direction: column; gap: 3px;">
                ...
                <input type="number" id="ngitung-inline-bayar" inputmode="numeric" placeholder="0"
                  oninput="ngitungHandleBayarInput()"
                  style="
            ...
            <div id="ngitung-inline-customer-container" class="hidden" style="display: flex; gap: 6px; margin-bottom: 10px;">
              <div style="flex: 1; display: flex; flex-direction: column; gap: 3px; min-width: 0;">
                ...
                <input type="text" id="ngitung-inline-customer" placeholder="Masukkan nama..."
                  style="
              ...
              <div style="width: 135px; display: flex; flex-direction: column; gap: 3px;">
                ...
                <input type="text" id="ngitung-inline-phone" inputmode="numeric" placeholder="08xx..."
                  style="
                    width: 100%; height: 32px;
                    background: rgba(0,0,0,0.35); border: 1px solid rgba(255,255,255,0.12);
                    color: #fff; padding: 0 8px; border-radius: 8px; font-size: 0.78rem;
            ...
            <div style="display: flex; gap: 6px; align-items: center;">
              <button onclick="ngitungProcessCheckout('save')" style="
                  ...
                  box-shadow: 0 4px 14px rgba(232,65,24,0.4);
                  transition: all 0.2s;
              ">💾 SIMPAN</button>
      ...
      function riwayat() {
        ...
        return `
          <div style="display: flex; gap: 8px; margin-bottom: 16px;">
            <button id="btn-tab-transaksi" onclick="window.switchRiwayatTab('transaksi')" style="padding: 6px 16px; font-size: 0.85rem; border-radius: 20px; font-weight: 600; cursor: pointer; border: 1px solid var(--garneta-cyan); background: var(--garneta-cyan); color: #000; transition: all 0.3s;">🧾 Riwayat Transaksi</button>
            <button id="btn-tab-hutang" onclick="window.switchRiwayatTab('hutang')" style="padding: 6px 16px; font-size: 0.85rem; border-radius: 20px; font-weight: 600; cursor: pointer; border: 1px solid var(--garneta-cyan); background: transparent; color: var(--neural-text); transition: all 0.3s;">💰 Bon (Kasbon)</button>
          </div>
...
Payung, Tepung, sak, 25, 170000, 8500"></textarea>
    ...
    const superAdminMenus = [
      ...
      ["pembelian", "🛒 Pembelian"], ["ngitung", "🧮 NGITUNG"], ["kalkulator", "📱 Kalkulator"], 
      ["penjualan", "💵 Penjualan"], ["riwayat", "🧾 Riwayat & Bon"], ["laporan", "📊 Laporan"],
  ...
  renderShell();
  console.log('renderShell success');
  try { render(); console.log('render success'); } catch (e) { console.log('Render Error:', e); }
  
  state.route = 'riwayat';
  try { render(); console.log('render riwayat success'); } catch (e) { console.log('Render Error:', e); }
