...
function bindGajiEvents() {
  ...
  if (formPayroll) {
    formPayroll.onsubmit = async (e) => {
      ...
      const views = { dashboard, "neural-hub": GARNETA STORE, barang, supplier, pembelian, ngitung, riwayat, hutang, kalkulator, penjualan, laporan, statistik, audit, users, gaji, settings };
...
Minyak Goreng 3 45000"></textarea>
    ...
    window.ngitungRenderTable = function() {
      ...
      window.ngitungRows.forEach((row, index) => {
        ...
        html += `
            <tr>
              <td colspan="3" style="width: 60%; padding: 4px;">
                <div style="display:flex;">
                  <input type="text" list="ngitung-history-list" value="${val}" oninput="ngitungParseAndUpdate(this, ${row.id})" onchange="ngitungFocusNext(this, ${row.id})" placeholder="Cth: terong 3000 5 (Nama Harga Banyak)" style="width: 100%; font-size: 0.8rem; font-weight: 700; padding: 6px 4px; border: none; border-bottom: none; border-radius: 0; background: transparent; color: #fff; text-transform: capitalize; box-shadow: none; outline: none;">
                </div>
              </td>
              <td class="row-amount" style="vertical-align: middle; font-weight: 800; width: 25%; font-size: 0.85rem; padding: 4px 12px 4px 4px; text-align: right;">${new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(amount)}</td>
              <td style="width: 15%; text-align: right; vertical-align: middle; padding: 4px 0 4px 4px;"><button class="btn danger" onclick="ngitungRemoveRow(${row.id})" ${window.ngitungRows.length === 1 ? 'disabled' : ''} style="background: transparent; border: none; font-size: 1.2rem; color: rgba(255,71,87,0.7); padding: 0; box-shadow: none;">🗑️</button></td>
            </tr>
        `;
      });
    ...
    window.ngitungAddRow = function() {
      ...
      if (tbody) {
        ...
        tr.innerHTML = `
            <td colspan="3" style="width: 60%; padding: 4px;">
              <div style="display:flex;">
                <input type="text" list="ngitung-history-list" value="" oninput="ngitungParseAndUpdate(this, ${newId})" onchange="ngitungFocusNext(this, ${newId})" placeholder="Cth: terong 3000 5" style="width: 100%; font-size: 0.8rem; font-weight: 700; padding: 6px 4px; border: none; border-bottom: none; border-radius: 0; background: transparent; color: #fff; text-transform: capitalize; box-shadow: none; outline: none;">
              </div>
            </td>
            <td class="row-amount" style="vertical-align: middle; font-weight: 800; width: 25%; font-size: 0.85rem; padding: 4px 12px 4px 4px; text-align: right;">Rp 0</td>
            <td style="width: 15%; text-align: right; vertical-align: middle; padding: 4px 0 4px 4px;"><button class="btn danger" onclick="ngitungRemoveRow(${newId})" style="background: transparent; border: none; font-size: 1.2rem; color: rgba(255,71,87,0.7); padding: 0; box-shadow: none;" title="Hapus">🗑️</button></td>
        `;
...
Beras Premium 1"></textarea>
    ...
    function settings() {
      ...
      const titles = {
        ...
        tema: ["Tema Dashboard", "Pilih tema Neural Hub, Cyber Dashboard, Dark Minimal, atau Ocean Deep."],
      ...
      return `<section class="settings-page">
        ...
        <div class="settings-tabs">
          ${settingsTabButton("tema", "TEMA", tab)}
          ${settingsTabButton("warna", "WARNA", tab)}
          ${settingsTabButton("api", "PUSAT API", tab)}
          ${settingsTabButton("logo", "LOGO & WATERMARK", tab)}
        ...
        <div class="theme-panel">
          <div class="api-section-title">🎨 Tema Dashboard</div>
          <p class="muted">Pilih tema yang sesuai dengan preferensi Anda. Tema akan diterapkan ke seluruh aplikasi.</p>
          <div class="theme-grid" style="grid-template-columns: repeat(2, 1fr); gap: 16px; margin-top: 16px;">
            ${renderThemeOption('neural', 'Neural Hub', 'Tema default dengan aksen cyan', '#0b1f24', '#24f0c7', tab)}
            ${renderThemeOption('cyber', 'Cyber Dashboard', 'Tema futuristik dengan aksen magenta', '#0a0a0f', '#ff00ff', tab)}
            ${renderThemeOption('dark', 'Dark Minimal', 'Tema gelap minimalis', '#0d0d0d', '#60a5fa', tab)}
            ${renderThemeOption('ocean', 'Ocean Deep', 'Tema biru laut yang tenang', '#0c1a2d', '#00d4ff', tab)}
          </div>
        </div>
        ` : ""}
        ...
        ${tab === "logo" ? `
        <div class="theme-panel">
          <div class="api-section-title">Logo & Watermark</div>
          <div class="brand-preview-grid">
            <div class="asset-preview" id="logo-preview"></div>
            <label>Upload Logo Sidebar / Dashboard
              <input id="brand-logo-upload" type="file" accept="image/png,image/jpeg,image/jpg,image/svg+xml,image/webp">
            </label>
          </div>
          <div class="brand-preview-grid">
            <div class="asset-preview" id="watermark-preview"></div>
            <label>Upload Gambar Transparan Halaman
              <input id="watermark-upload" type="file" accept="image/png,image/jpeg,image/jpg,image/svg+xml,image/webp">
            </label>
          </div>
          <label>Transparansi Gambar Halaman
            <div class="range-row">
              <input id="watermark-opacity" type="range" min="0" max="35" step="1">
              <strong id="watermark-opacity-label">14%</strong>
            </div>
          </label>
          <label>Ukuran Gambar Watermark
            <div class="range-row">
              <input id="watermark-size" type="range" min="20" max="95" step="1">
              <strong id="watermark-size-label">44%</strong>
            </div>
          </label>
          ...
          <label>Ukuran Teks Watermark
            <div class="range-row">
              <input id="watermark-text-size" type="range" min="1" max="9" step=".25">
              <strong id="watermark-text-size-label">4vw</strong>
            </div>
          </label>
          <label>Warna Teks Watermark
            <input id="watermark-text-color" type="color">
          </label>
          <div class="actions">
            <button class="api-primary" id="save-brand-assets" type="button">SIMPAN LOGO</button>
            <button class="btn danger" id="clear-watermark-text" type="button">HAPUS TEKS</button>
            <button class="btn soft" id="reset-brand-assets" type="button">RESET GAMBAR</button>
          </div>
          <p class="muted">Gunakan PNG transparan atau SVG agar logo terlihat rapi. Watermark disarankan transparansi 8-18% supaya tetap enak dipandang.</p>
        </div>
        ` : ""}
        ${tab === "backup" ? `
        <div class="theme-panel">
        ...
        ${tab === "install" ? `
        <div class="theme-panel">
          <div class="api-section-title">Install di HP</div>
          <p class="muted">Aplikasi sudah disiapkan sebagai PWA. Buka dari browser HP, lalu pilih Add to Home Screen / Install App.</p>
          <button class="btn soft" id="install-pwa" type="button">INSTALL APP</button>
        </div>
        ` : ""}
      </section>`;
    }
